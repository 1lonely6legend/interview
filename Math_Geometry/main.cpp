...
int main() {
  ...
  Point p1(1, 1), p2(4, 5);
  std::cout << "Distance between p1 and p2: " << GetDistance(p1, p2)
            << std::endl;  //5.0.
  Line line(Point(0, 0), Point(10, 0));
  Point p(3, 4);
  std::cout << "Distance between p and line: " << GetDistance(p, line)
            << std::endl;  //4.0.
  Segment seg(Point(5, 5), Point(9, 6));
  std::cout << "Distance between p and segment: " << GetDistance(p, seg)
            << std::endl;  //2.2367.
  ...
  Segment closestSegment =
      FindClosestSegmentByLinearSearch(testPoint, polyline);
